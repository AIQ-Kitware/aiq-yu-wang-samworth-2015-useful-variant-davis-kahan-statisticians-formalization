...
even when the population spectrum is well separated: for
`Σ = diag(50, 40, 30, 20, 10)` and `Σ̂ = diag(54, 37, 32, 23, 21)` at `r = s = 4`
it is zero, while the population gap at the same block is `10`.
...
with `d = s − r + 1`. Every hypothesis constrains `Σ` alone. That is what makes
the bound usable in statistics, and it is why the paper is cited across spectral
methods — principal component analysis, spectral clustering, covariance
estimation, and network models where an adjacency or Laplacian matrix
concentrates around a population version.
...
it: `SourcePopulationGap` identifies it as the greatest real satisfying the two
